// Package cfs reads RISC OS CFS (Compact File Store) single-file
// archives.
package cfs
...
// Archive implements archive.Archive for CFS files.
type Archive struct {
	f       *os.File
	entries []*archive.Entry
}

// Open opens a CFS archive at path.
...
// OpenFile returns a reader that decompresses the CFS block stream.
func (a *Archive) OpenFile(e *archive.Entry) (io.ReadCloser, error) {
	...
	return io.NopCloser(compress.NewCFSReader(lr)), nil
}

func (a *Archive) parse(path string) error {
	var skip, magic uint32
	if err := binary.Read(a.f, binary.LittleEndian, &skip); err != nil {
		return fmt.Errorf("reading cfs header: %w", err)
	}
	if err := binary.Read(a.f, binary.LittleEndian, &magic); err != nil {
		return fmt.Errorf("reading cfs magic: %w", err)
	}
	if magic != cfsMagic {
		return fmt.Errorf("invalid cfs magic 0x%x", magic)
	}

	var origlen, skipField, load, exec uint32
	if err := binary.Read(a.f, binary.LittleEndian, &origlen); err != nil {
		return fmt.Errorf("reading cfs origlen: %w", err)
	}
	if err := binary.Read(a.f, binary.LittleEndian, &skipField); err != nil {
		return fmt.Errorf("reading cfs skip field: %w", err)
	}
	if err := binary.Read(a.f, binary.LittleEndian, &load); err != nil {
		return fmt.Errorf("reading cfs load: %w", err)
	}
	if err := binary.Read(a.f, binary.LittleEndian, &exec); err != nil {
		return fmt.Errorf("reading cfs exec: %w", err)
	}
	...
	name := basename
	for _, suffix := range []string{",d96", ".d96"} {
